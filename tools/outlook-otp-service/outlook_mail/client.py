...
class OutlookIMAPClient:
    ...
    def fetch_recent(
        ...
        *,
        ...
        criteria = self._build_search_criteria(options)
    ...
    def _build_search_criteria(self, options: SearchOptions) -> list[str]:
        criteria: list[str] = ["UNSEEN" if options.unseen_only else "ALL"]
        if options.since:
